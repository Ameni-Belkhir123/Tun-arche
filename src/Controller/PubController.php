...
final class PubController extends AbstractController
{
    ...
    {
        $comments = $publication->getCommantaires(); // Récupère les commentaires associés à la publication

        return $this->render('pub/comments.html.twig', [
            'publication' => $publication,
            'comments' => $comments, // Passe les commentaires à la vue
        ]);
    }

    #[Route('/{id}/like', name: 'app_pub_like', methods: ['POST'])]
    ...
    #[Route('/{id}/unlike', name: 'app_pub_unlike', methods: ['POST'])]
    public function unlike(Publication $publication, EntityManagerInterface $entityManager): Response
    {
        $publication->incrementUnlikes();
        $entityManager->flush();

        return new JsonResponse(['likes' => $publication->getLikes(), 'unlikes' => $publication->getUnlikes()]);
    }
}